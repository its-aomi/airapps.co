...
use Merkulove\SpoterElementor\Unity\Cache;
use Merkulove\SpoterElementor\Unity\Helper;
use Merkulove\SpoterElementor\Unity\Plugin;
use Merkulove\SpoterElementor\UninstallHelper;
use Merkulove\SpoterElementor\Unity\EnvatoItem;
...
final class Uninstall {
    ...
    private function remove_settings() {
        ...
        foreach ( wp_load_alloptions() as $option => $value ) {
            if ( strpos( $option, 'mdp_spoter_elementor' ) === 0 ) {
                $settings[] = $option;
            }
        }
    ...
    public function get_uninstall_mode() {
        ...
        $uninstall_settings = get_option( 'mdp_spoter_elementor_migration_settings' );

        if ( isset( $uninstall_settings[ 'mdp_spoter_elementor_migration_settings' ] ) && $uninstall_settings[ 'mdp_spoter_elementor_migration_settings' ] ) { // Default value.
            $uninstall_settings = [
